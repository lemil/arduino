...
void Adafruit_StepperMotor::setSpeed(uint16_t rpm) {
  //out.println("steps per rev: "); out.println(revsteps);
  //out.println("RPM: "); out.println(rpm);
...
void Adafruit_StepperMotor::step(uint16_t steps, uint8_t dir,  uint8_t style) {
  ...
  if (style == INTERLEAVE) {
    ...
    steps *= MICROSTEPS;
#ifdef MOTORDEBUG
    out.print("steps = "); out.println(steps, DEC);
#endif
  }

  while (steps--) {
    //out.println("step!"); out.println(uspers);
...
#ifdef MOTORDEBUG
  out.print("current step: "); out.println(currentstep, DEC);
  out.print(" pwmA = "); out.print(ocra, DEC); 
  out.print(" pwmB = "); out.println(ocrb, DEC); 
#endif
...
#ifdef MOTORDEBUG
  out.print("Latch: 0x"); out.println(latch_state, HEX);
#endif
